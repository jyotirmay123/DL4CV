...
try:
    ...
    from utils.SpatialCrossMapLRN_temp import SpatialCrossMapLRN_temp
import os
import time
...
def CrossMapLRN(size, alpha, beta, k=1.0, gpuDevice=0):
    ...
    n = Lambda( lambda x,lrn=lrn: Variable(lrn.forward(x.data).cuda(gpuDevice), volatile=True) if x.data.is_cuda else Variable(lrn.forward(x.data), volatile=True) )
    return n
